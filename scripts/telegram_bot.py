...
from surfsup.comm.str_constants import (
    SURFSUP_WELCOME_MSG,
    SURFSUP_USAGE_MSG,
    SURFSUP_BIO_MSG,
)
...
def parse_set_activity_handler(message):
    if message.text.lower() == "yes":
        ...
        bot.reply_to(
            message,
            "Alright. When you want to edit your current activity, send /activity!",
        )
...
def parse_activity_handler(message):
    try:
        if message.text.lower() in ["1", "shortboarding"]:
            bot.reply_to(
                message,
                "Okay shredder, you're activity is set to shortboarding! [insert tip]",
            )
        else:
            msg = bot.reply_to(
                message,
...
def parse_user_name_handler(message):
    try:
        ...
        msg = bot.reply_to(
            message,
            f"Hello {message.text}! Please reply with "
...
def parse_user_activity_handler(message):
    try:
        if message.text == "1":
            ...
            msg = bot.reply_to(
                message,
                "Awesome! Looks like you love to rip! "
...
def parse_set_user_preferences_handler(message):
    ...
    if message.text.lower() == "yes":
        ...
        if message.chat.id in user_information:
            ...
            msg = bot.reply_to(message, "First off, what is your name?")
            bot.register_next_step_handler(msg, parse_user_name_handler)
    else:
        bot.send_message(
...
def parse_shortboard_max_radius_handler(message):
    try:
        ...
        bot.register_next_step_handler(msg, parse_shortboard_wave_height_handler)
    except Exception:
        trace = traceback.format_exc(limit=1)
        print(trace)
        bot.reply_to(message, "failure")
...
def parse_shortboard_wave_height_handler(message):
    try:
        preferences = ShortboardPreferences(
            surf_height=int(message.text),
            travel_distance=tmp_storage[message.chat.id]["max_radius"],
        )
...
def location_report(message):
    ...
    pin_location = Location(message.location.latitude, message.location.longitude)
    print(pin_location)
    msg = ""
    if chat_id in user_information:
        ...
        if isinstance(activity_preferences, ShortboardPreferences):
            bot.send_message(message.chat.id, "Looking for some spots nearest to you!")
