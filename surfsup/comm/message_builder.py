...
class MessageBuilder:
    ...
    def clean(self, obj: dict) -> str:
        ...
        return (
            f"Surf Report: {self.get_emoji(value)} {value}\n"
    ...
    def build_report_message(self, spot_name: str) -> str:
        ...
        if not self.is_spot(spot_name):
            ...
            return (
                f"ERROR: {spot_name} was invalid. Possible corrections:\n"
    ...
    def build_report_message_for_location(
        self, loc: Location, max_radius: int, max_height: int
    ):
        ...
        return "\n".join(
            [self.spot_message_fmt(mk_dict(spot)) for _, spot in best_fcsts.iterrows()]
        )
    ...
    def replace_apostrophe_lookalikes(self, spot_name: str) -> str:
        ...
        return spot_name.replace("`", "'").replace("‘", "'").replace("’", "'")
